...
def BinSizeAnalysis(data_file, bin_folder, rs_scheme, nr_irreps, the_irreps, t_min, max_bin_size, chosen_fit_range, chosen_bin_size, version, the_weight, **kwargs):
    ...
    correlator_folder = vfa.DIRECTORY_EXISTS(bin_folder + f'Single_correlators_{rs_scheme}/')
    fits_folder = vfa.DIRECTORY_EXISTS(bin_folder + f'Fits_{rs_scheme}_v{version}_tmax_{chosen_fit_range[1]}/')
    images_folder = vfa.DIRECTORY_EXISTS(bin_folder + 'Plots/')
    ...
    if not plots_only:
        ...
        if not os.listdir(correlator_folder):
            for bin_i in range(1, max_bin_size + 1):
                cs.SingleCorrelatorAnalysis(data_file, correlator_folder, version, rs_scheme,
                                                                        the_irreps,
                                                                        the_weight, nr_irreps, rebin_on='rb', rb=bin_i,
                                                                        kbt= nr_kbt,
                                                                        number_cfgs=nr_cfgs,
                                                                        own_kbt_list=kbt_samples)
