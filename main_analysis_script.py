...
if not ed.ensembles[myRuns.ensemble]['allConfigs']:
    ...
    myWeight = np.asarray(vfa.RW_NORMALIZATION([myWeight[ii] for ii in myTempCnfgs], myCnfgs), dtype=np.double)
...
### Ratio Multi-Hadrons
    ...
    if myRuns.corrs:
        ...
        ### Single-hadron corrs
        mySHVersion =  f'{myRuns.ensemble}_singles_{myRuns.version}' 
