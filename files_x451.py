...
### Reweighting factors ###OLD
# weight_raw_1 = np.array(np.loadtxt(location + '/data/X451/X451r001_rw_1.dat', unpack=True))[1] ##OLD
# weight_raw_2 = np.array(np.loadtxt(location + '/data/X451/X451r001_rw_2.dat', unpack=True))[1] ##OLD

# weight_raw = [] ##OLD
# for ii in range(len(weight_raw_1)): ##OLD
    # weight_raw.append(float(weight_raw_1[ii])*float(weight_raw_2[ii])) ##OLD
...
### Reweighting factors
pre_weight = np.array(vf.RW_NORMALIZATION(weight_raw,ncfgs), dtype=np.float64)


### Number of gauge configurations
if lambdas_analysis:
    ncfgs = np.array(f[name[0]+'/data']).shape[0] # lambdas
    # cnfgs_list = np.arange(39,2010,40) # lambdas T00 ## OLD: np.arange(40,2010,40)
    cnfgs_list = np.arange(19,2010,40) # lambdas T01 ## OLD: np.arange(20,2010,40)
    # cnfgs_list = np.arange(19,2010,20) # ALL t00 and t01 ## OLD: np.arange(20,2010,40)
else:
