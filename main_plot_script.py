...
myGevpFlag = True ### Plots the eigenvalues
...
if myEns == 'N451': from files_n451 import singleTMinsFitPlots, multiTMinsFitPlots, name, name1, the_hadron_state
elif myEns == 'N201': from files_n201 import singleTMinsFitPlots, multiTMinsFitPlots, name, name1, the_hadron_state
elif myEns == 'D200': from files_d200 import singleTMinsFitPlots, multiTMinsFitPlots, name, name1, the_hadron_state
elif myEns == 'X451': from files_x451 import singleTMinsFitPlots, multiTMinsFitPlots, name, name1, the_hadron_state
...
if myWhichCorrelator =='s':
    ...
    myMatrixCorrelatorData = h5py.File(myDataLocation + 'Matrix_correlators' + myChosenIsospin + myTypeRs + reBin +'_v%s.h5'%myVersion,'r')
    ...
    if plotCorrs: 
        pcorr.PlotMultiHadronCorrelators(myMatrixCorrelatorData, myChosenIsospin, myTypeRs, myVersion, myT0, myPlotLocation, reBin, nr_irreps=myNrIrreps, first_irrep=myFirstIrrep, last_irrep = myLastIrrep, diag_corrs= myDiagonalCorrs, gevp=myGevpFlag, ops_analysis=myOperatorsFlag)
    ...
    if joinPlots:
        ...
        for aa in irreps:
            ...
            for bb in range(len(ops)):
                ...
                ### Eigenvalues 
                if os.path.isfile(myPlotLocation + 'Eigenvalues' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Eigenvalues' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
                
                ### Eigenvalues log-plots
                if os.path.isfile(myPlotLocation + 'Eigenvalues' + myChosenIsospin + aa +  '_%s_log'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Eigenvalues' + myChosenIsospin + aa +  '_%s_log'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
                
                ### Histogram Eigenvlaues
                if os.path.isfile(myPlotLocation +'Histogram_Eigenvalues' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin +  '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation +'Histogram_Eigenvalues' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin +  '_v%s.pdf'%myVersion)
                
                ### Effective Mass Eigenvlaues
                if os.path.isfile(myPlotLocation + 'EffectiveMass_Eigenvalues' + myChosenIsospin+ aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'EffectiveMass_Eigenvalues' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
                
                ### Fits Eigenvalues
                if os.path.isfile(myPlotLocation + 'Tmin_Fits' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Tmin_Fits' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion)
                
                ### Chi^{2} Fits Eigenvalues 
                if os.path.isfile(myPlotLocation + 'Tmin_Chisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Tmin_Chisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion)
                
                ### Zoom Chi^{2} Fits Eigenvalues 
                if os.path.isfile(myPlotLocation + 'Tmin_Chisqr_Zoom' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Tmin_Chisqr_Zoom' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion)
                    
                 ### Total Chi^{2} Fits Eigenvalues 
                if os.path.isfile(myPlotLocation + 'Tmin_TotalChisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Tmin_TotalChisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion)
                    
                ### Delta Chi^{2} Fits
                if os.path.isfile(myPlotLocation + 'Tmin_DeltaChisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion):
                    x.append(myPlotLocation + 'Tmin_DeltaChisqr' + myChosenIsospin + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0)  + '_%sexp'%myNrExponentials + reBin + '_v%s.pdf'%myVersion)
            ...
            if os.path.isfile(myPlotLocation + 'ALLEigenvalues' + myChosenIsospin + aa + '_log'+ '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                x.append(myPlotLocation + 'ALLEigenvalues'+ myChosenIsospin + aa + '_log'+ '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
        
            ### Effective Masses all eigenvalues together
            if os.path.isfile(myPlotLocation + 'EffectiveMass_ALLEigenvalues' + myChosenIsospin + aa + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                x.append(myPlotLocation + 'EffectiveMass_ALLEigenvalues'+ myChosenIsospin + aa + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
                ...
                if any('Ops_chosen' in the_keys for the_keys in list(myMatrixCorrelatorData[aa+'/Operators_Analysis'].keys())):
                    ...
                    for the_op_item in the_list_of_chosen_ops:
                        ...
                        for bb in range(the_len_data):
                            if os.path.isfile(myPlotLocation + 'EffectiveMass_Eigenvalues' + myChosenIsospin + the_op_item +'_' + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                                x.append(myPlotLocation + 'EffectiveMass_Eigenvalues'+ myChosenIsospin + the_op_item +'_' + aa + '_%s'%str(bb) + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
                    
                    ### All eigenvalues together in one plot
                    if os.path.isfile(myPlotLocation + 'EffectiveMass_ALLEigenvalues' + myChosenIsospin + the_op_item +'_' + aa + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion):
                            x.append(myPlotLocation + 'EffectiveMass_ALLEigenvalues' + myChosenIsospin + the_op_item +'_' + aa + '_t0_%s'%str(myT0) + reBin + '_v%s.pdf'%myVersion)
            ...
            with open(myPlotLocation + myEns + myChosenIsospin + "_%s"%aa + "_t0%s"%str(myT0) + reBin + "_%s"%myTypeRs + "_v%s.pdf"%myVersion, "wb") as fout:
                merger.write(fout)
