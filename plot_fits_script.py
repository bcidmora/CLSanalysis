...
def PlotRatioMultiHadronsFits(the_multi_hadrons_fit_data, the_quantum_number, the_type_fit, the_nr_exps, the_type_rs, the_tmins, the_t0, the_version, the_location, the_rebin, **kwargs):  
    
    mr_irreps = list(the_multi_hadrons_fit_data.keys())
   ...
   ### How many irreps do you want to study        
    ...
    for the_irrep in mr_irreps:
        ...
        the_non_int = list(the_multi_hadrons_fit_data[f'{the_irrep}/Single_hadron_corrs'])
        the_op_list = list(the_multi_hadrons_fit_data[f'{the_irrep}/Operators'])
        ...
        if f'{the_nr_exps}exp' in list(the_multi_hadrons_fit_data[the_irrep].keys()): 
            ...
            for bb in range(len(the_op_list)):
                ...
                for nn in range(len(the_non_int)):
                
                    ### bb-th Eigenvalue
                    dis_set = np.asarray(the_data[f'lambda_{bb}_nonint_{nn}'])
                    ...
                    plt.errorbar(the_nt, the_fit_data, yerr=the_fit_sigmas, marker=vfp.the_markers_list[nn], ls='None', ms=6.5, markeredgewidth=1.5, lw=1.5, elinewidth=1.5, zorder=3, capsize=6, color=vfp.the_colors[nn], label=the_label)
